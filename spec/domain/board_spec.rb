...
RSpec.describe Board do
  ...
  describe '#receive_attack' do
    context 'when the coordinate hits a ship' do
      ...
      it 'returns HIT' do
        expect(subject.receive_attack(coordinate)).to eq Board::HIT
      end
    end
    ...
    context 'when the coordinate hits a ship and sinks it' do
      ...
      before do
        subject.receive_attack(Coordinate.new(1, 3))
        subject.receive_attack(Coordinate.new(1, 4))
      end

      it 'returns SUNK' do
        expect(subject.receive_attack(coordinate)).to eq Board::SUNK
      end
    end
    ...
    context 'when the coordinate misses' do
      ...
      it 'returns MISHIT' do
        expect(subject.receive_attack(coordinate)).to eq Board::MISHIT
      end
    end
    ...
    context 'when the coordinate has already been attacked' do
      ...
      it 'raises an ArgumentError' do
        expect { subject.receive_attack(coordinate) }.to raise_error(Board::CoordinateAlreadyAttackedError)
      end
    end
  end
  ...
  describe '#all_ships_sunk?' do
    context 'when all ships are sunk' do
      before do
        subject.receive_attack(Coordinate.new(4, 2))
        subject.receive_attack(Coordinate.new(4, 3))
        subject.receive_attack(Coordinate.new(1, 2))
        subject.receive_attack(Coordinate.new(1, 3))
        subject.receive_attack(Coordinate.new(1, 4))
      end
